...
public:
  ...
  /// @brief Part 1 requires a read of all labels of the top creates on each
  /// stack
  ...
  /// @param pretty Crates are presented in horizontal by default. Vertical
  /// stacks are prettier
  void draw() {
...
private:
  void ordered_move(unsigned int &quantity, unsigned int &source,
  ...
  void move(unsigned int &quantity, unsigned int &source,
            unsigned int &destination) {
    for (auto i = 0; i < quantity; i++) {
      auto crate = this->stacks[source]->back();
      this->stacks[source]->pop_back();
...
//--------------------------------------------------------------------------------
void process(const Instructions &instructions, const Crates &data,
             const std::string &labels, bool reserve_order = false) {
...
int main() {
  ...
  // reverse the order of input to simulate a queue of incoming crates for the
  // ship to load.
