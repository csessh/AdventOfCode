...
if __name__ == "__main__":
    ...
    for row, line in enumerate(data):
        ...
        for number in numbers:
            n = Number(value=int(number.group(0)), row=row, span=number.span())
        ...
        for sym in symbols:
            s = Symbol(value=sym.group(0), row=row, col=sym.span()[0])
    ...
    for symbol in all_the_symbols:
        ...
        if len(seen) == 2:
            ...
            print(
                f"({symbol.col},{symbol.row}) gear sees {seen[0].value} and {seen[1].value} --> {seen[0].value * seen[1].value}"
            )
